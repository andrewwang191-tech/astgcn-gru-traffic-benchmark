...
class ASTGCN_block(nn.Module):
    ...
    def __init__(self, DEVICE, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials, num_of_vertices, num_of_timesteps):
        ...
        self.temporal_rnn = nn.GRU(input_size=nb_chev_filter,hidden_size=nb_time_filter,batch_first=True)
    ...
    def forward(self, x):
        ...
        nb_time_filter = self.temporal_rnn.hidden_size
        z = spatial_gcn.permute(0, 1, 3, 2).contiguous().reshape(B * N, T, C)
        rnn_out, _ = self.temporal_rnn(z)
        rnn_out = rnn_out.reshape(B, N, T, nb_time_filter).permute(0, 3, 1, 2)

        if self.time_strides > 1:
            rnn_out = rnn_out[:, :, :, self.time_strides-1::self.time_strides]
       
        x_residual = self.ln(F.relu(x_residual + rnn_out).permute(0, 3, 2, 1)).permute(0, 2, 3, 1)
